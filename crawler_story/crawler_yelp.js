    ...
    try {
        ...
        if (buttonClicked) {
            ...
            if (_count[0]["count('id')"] == 0) {
                ...
                if (reviews.length > 0) {
                    let values = reviews.map((element, index) => {
                        newContent = element.content
                            ? element.content.replace(/'/g, "''")
                            : "";
                        return `('${element.fullname}', '${newContent}', 0, '${element.src}', '${record.relate_id}', '${record.id}')`;
                    });
...
async function getAllCrawlerDataBase(offset = 0) {
    const query = ` SELECT * FROM crawler_map WHERE is_crawler = 0 ORDER BY id ASC LIMIT 100 offset ${offset}`;
...
(async () => {
    ...
    await page.goto("https://www-yelp-com.translate.goog/biz/altthai-arlington-heights?_x_tr_sl=vi&_x_tr_tl=en&_x_tr_hl=vi&_x_tr_pto=wapp");
