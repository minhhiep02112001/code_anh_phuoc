...
async function newChat(page) {
    ...
    await page.waitForTimeout(1000); // Chờ một chút để ChatGPT sẵn sàng
}
...
async function getPost(offset = 0, limit = 100) {
    const query = `
        ...
        FROM st_post where  is_crawler_content=0 && is_status =3 LIMIT ${limit} offset ${offset}`;
    return database.query(query);
}

const _content_main = `Brand: [Brand]  
Yêu cầu search các dữ liệu về Brand restaurant france avis để đưa ra dữ liệu Search xong hãy viết lại dựa vào yêu cầu sau, Trả về kết quả chính luôn:
 - Ngôn ngữ: tiếng Pháp  
...
Yêu cầu: search các dữ liệu về Brand restaurant france để đưa ra dữ liệu Search xong hãy viết lại dựa vào yêu cầu sau 
 - Ngôn ngữ: tiếng pháp 
...
Tìm kiếm thông tin về brand restaurant france avis để đưa ra dữ liệu duy nhất (1), Tìm kiếm xong hãy viết lại dựa vào yêu cầu sau:
- Ngôn ngữ: tiếng Pháp 
- Cấu trúc: Brand - Nhà hàng tại tên đường, thành phố - 1 ẩm thực liên quan nhất | Resfra
- Yêu cầu chính: Viết tiêu đề SEO (META_TITLE) theo cấu trúc trên.
- Yêu cầu thêm: Trả luôn kết quả 1 dòng meta title,  bỏ qua các dàn ý, tiêu đề, bỏ các từ ngữ tóm tắt,link liên kết google, kết luận`;
...
(async () => {
    ...
    for (const element of listPost) {
        ...
        if (content_seo.length > 0) {
            content_seo = content_seo.replaceAll("Sources", "");
            await database.query(
                `UPDATE st_post SET meta_title  = '${content_seo}' WHERE id = ${element.id}`
            );
            console.log("Success meta_title: " + content_seo);
        }
        ...
        if (content.length > 0) {
            ...
            console.log("Success content: " + element.title);
        }
        ...
        if (content_about.length > 0) {
            ...
            console.log("Success content_about: " + element.title);
        }

        index++;
        let time_open = _content_time_open.replaceAll("[Brand]", element.title);
