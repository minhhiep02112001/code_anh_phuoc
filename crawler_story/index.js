...
async function loginAndAskChatGPT(data_content) {
    // Khởi tạo Puppeteer và mở trình duyệt
    const browser = await puppeteer.launch({
        headless: false,
        args: ["--start-maximized"], // Mở trình duyệt ở chế độ full màn hình
        defaultViewport: null, // Loại bỏ giới hạn khung nhìn mặc định
    }); // headless: false để thấy trình duyệt
    const page = await browser.newPage();
    await page.setViewport({ width: 1900, height: 1200 });
    // Điều hướng tới trang đăng nhập của OpenAI (ChatGPT)
    await page.goto("https://chat.openai.com/auth/login");

    // Đợi trang tải xong và nhấn vào nút "Log in"
    ...
    // Nhấn vào nút "Continue" (trước tiên chúng ta phải xử lý việc nút bị vô hiệu hóa)
    await page.evaluate(() => {
        const continueButton = document.querySelector(".continue-btn");
        if (continueButton) {
            continueButton.removeAttribute("disabled"); // Xóa thuộc tính disabled
        }
    });

    await page.waitForTimeout(1000);
    // Click vào nút Continue sau khi đã loại bỏ thuộc tính disabled
    await page.click(".continue-btn");
    // Đợi trường mật khẩu hiển thị và kiểm tra nếu nó sẵn sàng
    ...
    await page.evaluate(() => {
        ...
        if (passwordField) {
            passwordField.removeAttribute("disabled"); // Xóa thuộc tính disabled nếu có
        }
    });
    await page.type('input[name="password"]', "-tP?/g8,3s3xWLb", {
        delay: 100,
    }); // Gõ mật khẩu chậm lại

    // Xóa thuộc tính disabled của nút "Continue" cho bước đăng nhập
    await page.evaluate(() => {
    ...
    await page.waitForTimeout(1000);
    // Chờ nút "Continue" không còn bị disabled và click vào nó
    await page.waitForSelector('button[name="action"]:not([disabled])', {
        visible: true,
    });
    await page.click('button[name="action"]');

    // Đợi cho đến khi đăng nhập thành công và chuyển hướng đến giao diện ChatGPT
    await page.waitForNavigation({ waitUntil: "networkidle2" });

    // Tại giao diện chính của ChatGPT, nhập câu hỏi của bạn vào div #prompt-textarea
    await page.waitForSelector('div[id="prompt-textarea"]');

    // Đảm bảo không có bất kỳ sự kiện ngoài ý muốn nào trong quá trình nhập
    // Sao chép nội dung vào clipboard
    await page.evaluate((data_content) => {
        navigator.clipboard.writeText(data_content);
    }, data_content);
    await page.waitForTimeout(5000);
    // Dán toàn bộ nội dung từ clipboard vào ô nhập liệu
    ...
    await page.evaluate((data_content) => {
        ...
        // Kích hoạt sự kiện 'input' để đảm bảo ChatGPT nhận diện được nội dung mới
    ...
    // Đợi phản hồi của ChatGPT (có thể điều chỉnh tùy theo tình huống)
...
// Gọi hàm để thực hiện quá trình đăng nhập và gửi câu hỏi
// loginAndAskChatGPT();
...
async function getPost(offset = 0, limit = 10) {
    const query = `
        ...
        FROM st_post where is_status = 1 and is_content = 0 and 
    ...
    while (true) {
        try {
            ...
            for (let item of posts) {
                ...
                let content = await loginAndAskChatGPT(_content);
...
// Fonction pour extraire uniquement les balises HTML valides
function extractValidHtmlTags(html) {
    // Expression régulière pour trouver toutes les balises HTML
    ...
    while ((match = regex.exec(html)) !== null) {
        validTags.push(match[0]); // Ajouter la balise complète à la liste
    }
